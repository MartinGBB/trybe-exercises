...
const substituePersonagem = () => {
    return fs.readFile('./simpsonsFamily.json', 'utf-8')
    .then((result) => JSON.parse(result))
    .then((simpsons) => simpsons.filter((nelson) =>  nelson.id != "8"))
    .then((adicionaMaggie) => adicionaMaggie.concat([{ "id": "8", "name": "Maggie Simpson" }]))
    .then((sustitue) => fs.writeFile('./simpsonsFamily.json', JSON.stringify(sustitue)));
};
